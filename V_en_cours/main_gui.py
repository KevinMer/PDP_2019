...
class EcranFctMethod(GridLayout):
    ...
    def save(self, path, filename):
        ...
        if (self.choix == 0 and self.ids.les_onglets.current_tab.content.InfoParametre["code_conclu"]==0):
            conclu = 0
        if(self.choix==1 and  self.ids.les_onglets.current_tab.content.InfoParametre["code_conclu"]==1 ):
            conclu = 1
        if (self.choix == 0 and self.ids.les_onglets.current_tab.content.InfoParametre["code_conclu"]==1):
            conclu = 2
        if (self.choix == 1 and self.ids.les_onglets.current_tab.content.InfoParametre["code_conclu"]==0):
            conclu = 3
        else:
            conclu=self.ids.les_onglets.current_tab.content.InfoParametre["code_conclu"]
        print(self.ids.les_onglets.current_tab.content.InfoParametre["nom_projet"])
        try:
