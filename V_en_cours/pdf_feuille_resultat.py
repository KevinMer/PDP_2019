...
def diagnos(mot):
    ...
    if mot == "Contaminé" or mot == "NON":
        return Paragraph("<para align=center spaceb=3><font size=12><font color=red>" + mot + "</font></font></para>",
                         style)
    if mot == "Non contaminé" or mot == "OUI":
        return Paragraph("<para align=center spaceb=3><font size=12><font color=green>" + mot + "</font></font></para>",
                         style)
    if mot[0:4] == "Taux":
        mot = mot[20:]
        return Paragraph("<para align=center spaceb=3><font size=12><font color=red>" + mot + "</font></font></para>",
...
def creat_struct_pdf(Concordance_mf, Concordance_pf, Entite_d_Application, Emetteur, version):
    ...
    chu_titre = Paragraph(
        "<para align=center spaceb=3><b><font size=16><font color=white>Feuille de résultats Recherche de contamination maternelle Kit PowerPlex 16 ® </font></font></b></para>",
        styles["Title"])
    ...
    P0 = Paragraph(
        "<para align=center spaceb=3><b><font size=14><font color=darkblue> Étude de la contamination materno-foetale et de la bonne identité des ADN lors de la réalisation d’un diagnostic prénatal à l’aide du kit PowerPlex ® 16 System </font></font></b></para>",
        style)
    Titre = [[LOGO, P0, "", "", "", "", "", "", "", "", "", ""]]
    ...
    if Concordance_mf == "OUI":
        if Concordance_pf == "OUI" or Concordance_pf == "ABS":
            ...
            data = [[colonne("Marqueur"), colonne("Contamination materno-fœtale"), "", "", "",
                     colonne("Concordance des ADN paternelles et fœtals"), "", ""],
                    ["", colonne("Informativité"), colonne("Résultat"), colonne("Contamination"),
                     colonne("Détails M/F"), colonne("Concordance Père/Fœtus"), colonne("Détails allèles père"),
                     colonne("Détails allèles fœtus")],
                    [colonne_marqueur("CSF1PO"), "", "", "", "", "", "", ""],
                    [colonne_marqueur("D13S317"), "", "", "", "", "", "", ""],
                    [colonne_marqueur("D16S539"), "", "", "", "", "", "", ""],
                    [colonne_marqueur("D18S51"), "", "", "", "", "", "", ""],
        ...
        if Concordance_pf == "OUI" or Concordance_pf == "ABS":
            data = [[colonne("Marqueurs"), colonne("Concordances des \nADN maternelles et fœtal"),
                     colonne("Détails allèle mère"), colonne("Détails allèle fœtus")],
                    [colonne_marqueur("CSF1PO"), "", "", ""],
                    [colonne_marqueur("D13S317"), "", "", ""],
                    [colonne_marqueur("D16S539"), "", "", ""],
                    [colonne_marqueur("D18S51"), "", "", ""],
            ...
            data = [[colonne("Marqueur"), colonne("Concordance des\n ADN maternelles et fœtals"),
                     colonne("Détails allèles mère"), colonne("Détails allèles fœtus"),
                     colonne("Concordance des \nADN paternelles et fœtals"), colonne("Détails allèles père"),
                     colonne("Détails allèles fœtus")],
                    [colonne_marqueur("CSF1PO"), "", "", "", "", "", ""],
                    [colonne_marqueur("D13S317"), "", "", "", "", "", ""],
                    [colonne_marqueur("D16S539"), "", "", "", "", "", ""],
...
def disposition_pdf(CHU_HEADER, HEADER, nom_utilisateur, t, canv, Concordance_mf, Concordance_pf, Contamination,
    ...
    if Concordance_mf != "NON":
        P_nb_Nconta = Paragraph(
            "<b><font size=12><font color=darkblue>Nombre de marqueurs informatifs non contaminés : </font><font color=green>" + str(
                nb_info_Nconta) + "</font></font></b>", style)
        P_nb_conta = Paragraph(
            "<b><font size=12><font color=darkblue>Nombre de marqueurs informatifs contaminés : </font><font color=red>" + str(
                nb_info_Conta) + "</font></font></b>", style)
        P_moy = Paragraph(
            "<font size=12><b><font color=darkblue>Moyenne du pourcentage de contamination : </font>" + str(
                moy_conta) + "</b></font>", style)
    else:
        P_nb_Nconta = Paragraph(
            "<b><font size=12><font color=darkblue>Nombre de marqueurs informatifs non contaminés : </font><font color=red>" + str(
                nb_info_Nconta) + "</font></font></b>", style)
        P_nb_conta = Paragraph(
            "<b><font size=12><font color=darkblue>Nombre de marqueurs informatifs contaminés : </font><font color=red>" + str(
                nb_info_Conta) + "</font></font></b>", style)
        P_moy = Paragraph(
            "<font size=12><b><font color=darkblue>Moyenne du pourcentage de contamination : </font><font color=red>" + str(
                moy_conta) + "</font></b></font>", style)

    P_conta_echantillon = Paragraph(
        "<font size=12><b><font color=darkblue>Contamination : </font>" + posinega(Contamination) + "</b></font>",
        style)
    P_seuil_m = Paragraph(
        "<font size=12><font color=darkblue>Seuil minimum de marqueur : </font>" + str(seuil_marqueur) + "</font>",
        style)
    P_seuil_h = Paragraph(
        "<font size=12><font color=darkblue>Seuil de hauteur d'un pic: </font>" + str(seuil_pic) + "</font>", style)
    P_seuil_p = Paragraph("<font size=12><font color=darkblue>Seuil pourcentage de contamination: </font>" + str(
        seuil_pourcentage) + "</font>", style)
