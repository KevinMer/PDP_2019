...
class EcranFctMethod(GridLayout):
    ...
    def load(self, path, filename):
        ...
        M, F, P, Echantillon_F, log = Traitement3.lecture_fichier(os.path.join(path, filename[0]))
        ...
        Traitement3.Echantillon.analyse_donnees(Echantillon_F, M, F, log)
        ...
        contenu_res = ResAnalyse(
            titre="Echantillon: " + donnees["Sample Name"][1] + "\n" + "\n", NvGroupe=len(self.ids.les_onglets.tab_list),show_save=self.show_save)
    ...
    def save(self, path, filename):
        with open(os.path.join(path, filename), 'w') as stream:
            stream.write(" \n")
        pdf_feuille_resultats.creation_PDF(nom_projet, nom_fichier_mere, nom_fichier_foetus, nom_fichier_pere, Sexe, dataframe, det_dataframe, choix_utilisateur)
        self.dismiss_popup()
