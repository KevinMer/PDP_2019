...
def calibration(temps_migration,intensite_fluo):
        ...
        """ graphe = plt.figure()
        ax = plt.subplot(111)
        ax.plot(calibrage_x,calibrage_y,label="a ="f'{lr[0]}')
        ax.plot(calibrage_x,calibrage_y,label="b = "f'{lr[1]}')
        ax.plot(calibrage_x,calibrage_y,label="Rcarre = "f'{lr[2]}')
...
if __name__ == "__main__":
        ...
        xFl_c = conversion_pb(xFl,a,b)
        xJoe_c = conversion_pb(xJoe,a,b)
        xTmr_c = conversion_pb(xTmr,a,b)
        """ plt.plot(xMarqueur_c,yMarqueur)
        plt.xlabel("Taille (pb)")
        plt.ylabel("Fluorescence (RFU/100)")
        plt.savefig("marqueurTcalibre.png") """
